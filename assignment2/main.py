...
# Print system details before testing functions
print(f"Operating System: {os.name}")  # Prints the OS name (e.g., 'nt' for Windows, 'posix' for Linux/macOS)
print(f"Python Version: {platform.python_version()}")  # Prints the Python version
...
while input_invalid and i in range(5):
    try:
        print("    ------------------------------------------------------------------")
        # Input for Hero's combat strength
        print("    |", end="    ")
        combat_strength = int(input("Enter your combat Strength (1-6): "))

        # Input for Monster's combat strength
        print("    |", end="    ")
        monster.combat_strength = int(input("Enter the monster's combat Strength (1-6): "))

        # Validate if inputs are in the valid range (1-6)
        if combat_strength not in range(1, 7):
            raise ValueError("Hero's combat strength must be between 1 and 6.")
        if monster.combat_strength not in range(1, 7):
            raise ValueError("Monster's combat strength must be between 1 and 6.")

        # If inputs are valid, set and break out of the loop
        input_invalid = False
...
# Handle case when too many invalid inputs are made
if input_invalid:
    print("    |    Too many invalid attempts. Exiting...")
else:
    print("    |    Valid inputs received.")
    print(f"    |    Hero's Combat Strength: {combat_strength}")
    print(f"    |    Monster's Combat Strength: {monster.combat_strength}")
...
while monster.health_points > 0 and health_points > 0:
    # Fight Sequence
    print("    |", end="    ")
    ...
    if not (attack_roll % 2 == 0):
        ...
        functions.save_battle_history("Hero",combat_strength,monster.combat_strength,health_points,monster.health_points)
        functions.load_battle_history()
        if monster.health_points == 0:
            ...
            try:
                # Call monster_attacks() and validate arguments
                health_points = functions.monster_attacks(monster.combat_strength, health_points)
                functions.save_battle_history("Monster",combat_strength,monster.combat_strength,health_points,monster.health_points)
                functions.load_battle_history()
            except ValueError as e:
